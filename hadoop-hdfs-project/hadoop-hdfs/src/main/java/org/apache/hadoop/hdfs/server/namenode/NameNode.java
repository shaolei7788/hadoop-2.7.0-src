 ...
 *       namenode为了快速相应用户的操作请求，所以把元数据加载到了内存里面
...
public class NameNode implements NameNodeStatusMXBean {
  ...
  protected void initialize(Configuration conf) throws IOException {
    ...
    // NameNodeRPCserver 里面有两个主要的RPC服务：
    ...
    //如果是NAMENODE角色，设置namenode地址和fsImage
    if (NamenodeRole.NAMENODE == role) {
  ...
  private void startCommonServices(Configuration conf) throws IOException {
	//TODO 创建NameNodeResourceChecker、激活BlockManager等
    namesystem.startCommonServices(conf, haContext);
    registerNNSMXBean();
    // 角色非`NamenodeRole.NAMENODE`的在此处启动HttpServer，提供HDFS的UI
    if (NamenodeRole.NAMENODE != role) {
      startHttpServer(conf);
      httpServer.setNameNodeAddress(getNameNodeAddress());
      httpServer.setFSImage(getFSImage());
  ...
  protected NameNode(Configuration conf, NamenodeRole role) 
    ...
    try {
      //设置dfs.nameservice.id、dfs.ha.namenode.id、NAMENODE——SPECIFY_KEYS等参数
  ...
  public static NameNode createNameNode(String argv[], Configuration conf)
    ...
    switch (startOpt) {
      //调用format方法对当前NameNode进行格式化，
      //根据参数可确定是否强制及交互式执行。NameNode格式化会将fsImage和edit以及所有数据块删除掉
      case FORMAT: {
  ...
  /**
   *
   *todo 启动了hadoop2.0的HA机制之后，SecondaryNameNode，CheckpointNode，BackupNode这些都不需要了
   * NameNode 在执行 HDFS 客户端提交的创建文件或者移动文件这样的写操作的时候，
   * 会首先把这些操作记录在 EditLog 文件之中，然后再更新内存中的文件系统镜像。
   ...
   * 其文件名形如 edits_${start_txid}-${end_txid}，其中${start_txid} 表示这个 segment 的起始事务 id，${end_txid}
   * 表示这个 segment 的结束事务 id，例如上图中的 edits_0000000000000000001-0000000000000000019
   */
  ...
  public static void main(String argv[]) throws Exception {
    ...
    try {
      ...
      if (namenode != null) {
        ...
        //调用RpcServer的join方法，表示等待RpcServer关闭 通常情况下这个服务是循环执行的
        namenode.join();
      }
    } catch (Throwable e) {
